...
- Repository-specific override: unless the user explicitly requests another target, automatic backup pushes must prefer `origin/backup/repro-local` even when the local branch name differs.
- Do not push to `origin/backup/repro-local-sanitized` unless the user explicitly asks; it is a legacy backup branch slated for removal.
...
- 本仓库特定规则：除非用户明确指定其他目标，自动备份推送必须优先使用 `origin/backup/repro-local`，即使当前本地分支名称不同。
- 不要推送到 `origin/backup/repro-local-sanitized`，除非用户明确要求；该分支是计划删除的旧备份分支。
- push 前必须展示将提交的文件列表。
