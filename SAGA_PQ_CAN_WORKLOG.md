...
状态：`已完成`（第一阶段：`saga/security_kernel.py` 已新增 `ModelRefinementMapping` 与 `model_refinement_mappings()`，将模型谓词映射到 Python 符号、证据测试、TCB 假设、excluded paths、toy cryptography limitation 与 protected sinks）
...
- P5. 建立轻量形式化模型，证明抽象命题 `Execute(surface) => N_verify=1 AND scope_ok AND replay_ok AND delegation_ok AND policy_ok`：`已完成`（第一阶段：Python state exploration 已落地到 `proofs/strict_runtime_auth_model.py`；TLA+ guarded transition / invariant 规格已落地到 `proofs/tla/StrictRuntimeAuth.tla`；symmetry-reduced layered companion model 已落地到 `proofs/tla/StrictRuntimeAuthLayered.tla`；测试覆盖 claim、scope-check mutation 反例、TLA+ artifact consistency、single-surface bounded TLC smoke、two-surface coexistence smoke、per-surface TLC decomposition runner 与 layered partition coverage）
- P6. 编写模型到 Python 实现的 refinement 对照表，明确 TCB、excluded paths、toy cryptography limitation 与论文表述边界：`已完成`（第一阶段：`ModelRefinementMapping` 已映射模型项到 Python 符号、证据测试、TCB、排除路径和 protected sinks）
...
2. 若要归档远端 proof-hardening artifact，需要注意当前分支映射后的新边界：
   ...
   - 当前环境没有 `gh`，也未发现 `GH_TOKEN` / `GITHUB_TOKEN` / `GITHUB_PAT` 等可用 API token；远端 workflow 触发仍需要 GitHub UI 或可用 CLI/API 认证。
   - 满足触发条件后，建议先触发 `skip_mutations=true` 快速 proof-test-only workflow，再考虑完整 mutation evidence workflow。
3. 若继续强化形式化证据，可进一步做 refinement mapping 到 `StrictRuntimeAuthLayered.tla` 的逐 layer 对照，或增加更细的 delegation/replay 子模型。
4. 若切回系统工程主线，可选择 ML-DSA adapter 真实 backend 接线、Redis/PostgreSQL replay backend artifact、CNN + Ring/Module-LWE 路线评估或更多 live sample；这些方向仍暂后置，除非用户重新指定。
...
- 因为用户指定的是“旧 `origin/repro-local` -> `main`，旧 `origin/backup/repro-local` -> `repro-local`”，所以 proof-hardening workflow 当前不在 `origin/main`。
- 若 GitHub Actions 默认页需要显示 `Optional proof-hardening`，下一步需要将 GitHub 默认分支改为 `repro-local`，或把 proof-hardening workflow 及其依赖单独合入 `main`。
