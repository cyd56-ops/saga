...
1. 若要归档远端 proof-hardening artifact，需要先满足远端触发前置条件：
   - `.github/workflows/proof-hardening.yml` 需进入 GitHub 可触发分支；当前它存在于 `origin/backup/repro-local`，但不在 `origin/main` 或 `origin/repro-local`。
   - 当前环境没有 `gh`，也未发现 `GH_TOKEN` / `GITHUB_TOKEN` / `GITHUB_PAT` 等可用 API token；需要用户在 GitHub UI 手动触发，或提供可用 GitHub CLI / API 认证。
   - 满足上述条件后，建议先触发 `skip_mutations=true` 快速 proof-test-only workflow，再考虑完整 mutation evidence workflow。
2. 若继续强化形式化证据，可进一步做 refinement mapping 到 `StrictRuntimeAuthLayered.tla` 的逐 layer 对照，或增加更细的 delegation/replay 子模型。
3. 若切回系统工程主线，可选择 ML-DSA adapter 真实 backend 接线、Redis/PostgreSQL replay backend artifact、CNN + Ring/Module-LWE 路线评估或更多 live sample；这些方向仍暂后置，除非用户重新指定。
