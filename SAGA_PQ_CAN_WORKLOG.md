...
3. 若要归档远端 proof-hardening artifact，需要注意当前分支映射后的新边界：
...
4. 若继续强化形式化证据，可进一步做 refinement mapping 到 `StrictRuntimeAuthLayered.tla` 的逐 layer 对照，或增加更细的 delegation/replay 子模型。
5. 若切回系统工程主线，可选择 ML-DSA adapter 真实 backend 接线、Redis/PostgreSQL replay backend artifact、CNN + Ring/Module-LWE 路线评估或更多 live sample；这些方向仍暂后置，除非用户重新指定。
...
- 本轮不自动推送主开发分支；若未命中敏感路径，可按工作流推送到 `origin/backup/repro-local`。
