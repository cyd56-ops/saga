...
7. delegation/replay 细化子模型已完成，并已有覆盖该子模型的 GitHub-hosted proof-hardening artifact：
   ...
   - 若继续论文工程，可优先把 proof evidence summary 的 refinement / layer / submodel 信息接入论文表格或附录生成脚本。
   - 若继续分支同步流程，应在确认当前 backup 分支内容后，再由用户明确决定是否把 `origin/backup/repro-local` 同步到默认分支 `origin/repro-local`。
